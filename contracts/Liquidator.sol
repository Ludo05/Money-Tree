...
/**
 * @title MoneyTreeFactory Liquidator contract.
 */
contract Liquidator {
    ...
    string private constant ONLY_MONEYTREE_FACTORY = "ONLY_MONEYTREE_FACTORY";
    ...
    constructor(address _tokenAddr, address payable  _MoneyTreeFactoryAddr){
        bank = MoneyTreeFactory(_MoneyTreeFactoryAddr);
        token = MoneyTree(_tokenAddr);
    }

    /**
     * @notice Withdraw MoneyTree.
     */
    function withdraw()
    external
    {
     ...
     * @param collateral The amount of the loan collateral.
    ...
    {
        require(liquidations[liquidationId].endTime <= block.timestamp, OPEN_LIQUIDATION);
        require(liquidations[liquidationId].bestBid != 0, NO_BID);
        liquidations[liquidationId].state = LiquidationState.FINISHED;
        token.burn(msg.sender, liquidations[liquidationId].amount);
    ...
    onlyActive(liquidationId)
    {
        ...
        if (token.transferFrom(msg.sender, address(this), allowance)) {
            deposits[msg.sender] = deposits[msg.sender] + (allowance);
        }
        require(liquidations[liquidationId].amount <= deposits[msg.sender], INSUFFICIENT_FUNDS);
        deposits[msg.sender] = deposits[msg.sender] - (liquidations[liquidationId].amount);
        deposits[liquidations[liquidationId].bestBidder] = deposits[liquidations[liquidationId].bestBidder] + (liquidations[liquidationId].amount);
        liquidations[liquidationId].bestBidder = payable(msg.sender);
    ...
    /**
     * @dev Throws if called by any account other than our MoneyTreeFactory smart conrtact.
     */
    modifier onlyMoneyTreeFactory() {
        require(msg.sender == address(bank), ONLY_MONEYTREE_FACTORY);
        _;
    }
}
